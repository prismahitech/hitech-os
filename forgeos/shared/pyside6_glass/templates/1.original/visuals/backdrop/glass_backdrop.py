...
def _glass_palette(theme_id: str, variant: str = "selector") -> _GlassPalette:
    ...
    if silver_theme:
        ...
        wash = _qcolor_from_value("#eef6ff", 0.024 if selector_variant else 0.028)
        border = _qcolor_from_value("#e8f6ff", 0.22 if selector_variant else 0.18)
        line = _qcolor_from_value("#8cefff", 0.07)
        sheen = _qcolor_from_value("#ffffff", 0.11)
        orb_a = _qcolor_from_value("#eff7ff", 0.20 if selector_variant else 0.16)
        orb_b = _qcolor_from_value("#8cefff", 0.17 if selector_variant else 0.14)
        orb_c = _qcolor_from_value("#d7e1ff", 0.13 if selector_variant else 0.10)
    ...
    wash = _qcolor_from_value(mix_hex(t["header_fill"], t["legend_fill"], 0.44 if dark else 0.20), 0.11 if dark else 0.54)
    border = _qcolor_from_value(mix_hex(t["focus"], t["legend_stroke"], 0.30 if dark else 0.12), 0.24 if dark else 0.28)
    line = _qcolor_from_value(t["header_stroke"], 0.22 if dark else 0.18)
    sheen = _qcolor_from_value("#ffffff", 0.20 if dark else 0.16)
    orb_a = _qcolor_from_value(t["halo_a"], 0.42 if selector_variant and dark else 0.30 if selector_variant else 0.34 if dark else 0.12)
    orb_b = _qcolor_from_value(t["halo_b"], 0.34 if selector_variant and dark else 0.24 if selector_variant else 0.28 if dark else 0.10)
    orb_c = _qcolor_from_value(t["focus"], 0.28 if dark else 0.09)
    sparkle = _qcolor_from_value("#ffffff", 0.68 if dark else 0.42)
    star_soft = _qcolor_from_value("#ffffff", 0.30 if dark else 0.18)
    star_bright = _qcolor_from_value("#ffffff", 0.66 if dark else 0.34)
    return _GlassPalette(
        canvas_top=canvas_top,
        canvas_bottom=canvas_bottom,
        wash=wash,
...
class FrostedGlassBackdrop(QWidget):
    ...
    def _resolve_motion_interval_ms(self) -> int:
        ...
        if window is not None:
            try:
                ...
                is_active = True
        silver = _is_silver_theme_id(self._theme_id)
        if silver:
            return 24 if is_active else 48
        return 32 if is_active else 60

    def _sync_motion_timer(self) -> None:
        should_run = self._motion_enabled and self.isVisible()
        if not should_run:
    ...
    def _ensure_static_cache(self, rect: QRectF) -> QPixmap:
        ...
        if _is_silver_theme_id(self._theme_id):
            ...
            top_wash.setColorAt(0.0, QColor(255, 255, 255, 30))
            top_wash.setColorAt(0.38, QColor(156, 224, 255, 14))
            ...
            painter.setPen(QPen(self._palette.border, 1.25))
            ...
            painter.setPen(QPen(self._palette.sheen, 1.18))
            painter.drawPath(sheen_path)
        else:
            top_wash = QLinearGradient(static_rect.left(), static_rect.top(), static_rect.right(), static_rect.bottom())
            top_wash.setColorAt(0.0, QColor(255, 255, 255, 30))
            top_wash.setColorAt(0.40, QColor(124, 205, 255, 18))
            top_wash.setColorAt(1.0, QColor(255, 255, 255, 0))
            painter.fillRect(static_rect, top_wash)
            ...
            painter.setPen(QPen(self._palette.border, 1.24))
            painter.drawPath(glass_path)

            sheen_path = QPainterPath()
            sheen_path.moveTo(static_rect.width() * 0.10, static_rect.height() * 0.11)
            sheen_path.cubicTo(
                static_rect.width() * 0.28,
                static_rect.height() * 0.05,
                static_rect.width() * 0.58,
                static_rect.height() * 0.13,
                static_rect.width() * 0.84,
                static_rect.height() * 0.07,
            )
            painter.setPen(QPen(self._palette.sheen, 1.08))
            painter.drawPath(sheen_path)
