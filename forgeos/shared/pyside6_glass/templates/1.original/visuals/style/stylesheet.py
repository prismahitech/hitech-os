...
def build_stylesheet(theme_id: str, scale_id: str = "100") -> str:
    ...
    shell_top = with_alpha(mix_hex(t["header_fill"], t["legend_fill"], 0.28 if dark else 0.12), 0.38 if dark else 0.60)
    shell_bottom = with_alpha(mix_hex(t["canvas_bg"], t["header_fill"], 0.18 if dark else 0.06), 0.24 if dark else 0.48)
    shell_border = with_alpha(mix_hex(t["focus"], t["legend_stroke"], 0.22 if dark else 0.10), 0.22 if dark else 0.26)
    shell_rim = with_alpha(t["focus"], 0.28 if dark else 0.32)
    shell_edge_top = with_alpha("#ffffff", 0.09 if dark else 0.18)
    shell_edge_side = with_alpha(mix_hex(t["focus"], t["legend_stroke"], 0.18 if dark else 0.12), 0.13 if dark else 0.20)
    shell_edge_bottom = with_alpha(mix_hex(t["canvas_bg"], t["focus"], 0.22 if dark else 0.10), 0.11 if dark else 0.18)
    shell_hover_top = with_alpha("#ffffff", 0.12 if dark else 0.24)

    hero_top = with_alpha(mix_hex(t["focus"], t["header_fill"], 0.12 if dark else 0.05), 0.14 if dark else 0.36)
    hero_bottom = with_alpha(mix_hex(t["legend_fill"], t["canvas_bg"], 0.14 if dark else 0.04), 0.28 if dark else 0.48)
    hero_border = with_alpha(t["focus"], 0.24 if dark else 0.34)
    hero_edge_top = with_alpha("#ffffff", 0.09 if dark else 0.18)
    hero_edge_bottom = with_alpha(mix_hex(t["focus"], t["canvas_bg"], 0.18 if dark else 0.10), 0.09 if dark else 0.18)

    card_top = with_alpha(mix_hex(t["legend_fill"], t["header_fill"], 0.08 if dark else 0.04), 0.24 if dark else 0.46)
    card_bottom = with_alpha(mix_hex(t["canvas_bg"], t["legend_fill"], 0.14 if dark else 0.04), 0.16 if dark else 0.36)
    muted_top = with_alpha(mix_hex(t["canvas_bg"], t["legend_fill"], 0.16 if dark else 0.05), 0.18 if dark else 0.38)
    muted_bottom = with_alpha(mix_hex(t["canvas_bg"], t["header_fill"], 0.12 if dark else 0.04), 0.14 if dark else 0.32)
    footer_top = with_alpha(mix_hex(t["header_fill"], t["canvas_bg"], 0.12 if dark else 0.05), 0.18 if dark else 0.38)
    footer_bottom = with_alpha(mix_hex(t["canvas_bg"], t["legend_fill"], 0.12 if dark else 0.05), 0.12 if dark else 0.28)
    ...
    card_edge_top = with_alpha("#ffffff", 0.12 if dark else 0.16)
    card_edge_bottom = with_alpha(mix_hex(t["focus"], t["canvas_bg"], 0.18 if dark else 0.08), 0.12 if dark else 0.14)
    muted_edge_top = with_alpha("#ffffff", 0.10 if dark else 0.14)
    muted_edge_bottom = with_alpha(mix_hex(t["header_stroke"], t["canvas_bg"], 0.16 if dark else 0.06), 0.10 if dark else 0.12)
    footer_edge_top = with_alpha("#ffffff", 0.10 if dark else 0.14)
    footer_edge_bottom = with_alpha(mix_hex(t["focus"], t["canvas_bg"], 0.14 if dark else 0.06), 0.10 if dark else 0.12)
    ...
    chrome_bg_top = with_alpha(mix_hex(t["header_fill"], t["canvas_bg"], 0.22 if dark else 0.05), 0.46 if dark else 0.60)
    chrome_bg_bottom = with_alpha(mix_hex(t["legend_fill"], t["canvas_bg"], 0.18 if dark else 0.04), 0.34 if dark else 0.50)
    chrome_border = with_alpha(t["legend_stroke"], 0.16 if dark else 0.24)
    chrome_edge_top = with_alpha("#ffffff", 0.12 if dark else 0.16)
    chrome_edge_bottom = with_alpha(mix_hex(t["focus"], t["canvas_bg"], 0.12 if dark else 0.08), 0.10 if dark else 0.14)
    ...
    input_bg = with_alpha(mix_hex(t["canvas_bg"], t["legend_fill"], 0.16 if dark else 0.04), 0.24 if dark else 0.64)
    input_fg = value
    input_border = with_alpha(t["legend_stroke"], 0.14 if dark else 0.30)
    input_hover = with_alpha(t["focus"], 0.40 if dark else 0.50)
    input_focus = with_alpha(t["focus"], 0.80 if dark else 0.82)
    input_focus_bg = with_alpha(mix_hex(t["canvas_bg"], t["header_fill"], 0.12 if dark else 0.06), 0.30 if dark else 0.72)
    input_focus_glow = with_alpha(t["focus"], 0.22 if dark else 0.22)
    input_disabled_fg = with_alpha(t["muted_text"], 0.82)
    input_disabled_bg = with_alpha(mix_hex(t["canvas_bg"], t["legend_fill"], 0.12 if dark else 0.04), 0.18 if dark else 0.58)
    input_disabled_border = with_alpha(t["legend_stroke"], 0.08 if dark else 0.16)
    dropdown_bg = with_alpha(mix_hex(t["legend_fill"], t["canvas_bg"], 0.08 if dark else 0.03), 0.38 if dark else 0.82)
    dropdown_panel_top = with_alpha(mix_hex(t["header_fill"], t["legend_fill"], 0.18 if dark else 0.04), 0.44 if dark else 0.84)
    dropdown_panel_bottom = with_alpha(mix_hex(t["canvas_bg"], t["legend_fill"], 0.20 if dark else 0.04), 0.28 if dark else 0.72)
    dropdown_border = with_alpha(t["focus"], 0.20 if dark else 0.18)
    selection_bg = t["focus"]
    selection_fg = t["chip_light"]
    dropdown_hover = with_alpha(t["focus"], 0.10 if dark else 0.14)
    dropdown_hover_border = with_alpha(t["focus"], 0.18 if dark else 0.18)
    dropdown_selection_top = with_alpha(t["focus"], 0.22 if dark else 0.28)
    dropdown_selection_bottom = with_alpha(mix_hex(t["focus"], t["chip_light"], 0.08 if dark else 0.10), 0.14 if dark else 0.20)
    dropdown_selection_border = with_alpha(t["focus"], 0.28 if dark else 0.24)
    dropdown_selection = with_alpha(t["focus"], 0.16 if dark else 0.24)
    perf_warn = with_alpha(t["warning_stroke"], 0.74 if dark else 0.72)
    perf_error = with_alpha("#ff9c9c", 0.82 if dark else 0.78)
    ...
    if silver_theme:
        shell_top = with_alpha("#f2f8ff", 0.08)
        shell_bottom = with_alpha("#8ea5bf", 0.04)
        shell_border = with_alpha("#eef8ff", 0.24)
        shell_rim = with_alpha(t["focus"], 0.48)
        shell_edge_top = with_alpha("#ffffff", 0.28)
        shell_edge_side = with_alpha("#dff6ff", 0.18)
        shell_edge_bottom = with_alpha("#77dff4", 0.12)
        shell_hover_top = with_alpha("#ffffff", 0.34)

        hero_top = with_alpha("#f2f8ff", 0.12)
        hero_bottom = with_alpha(t["focus"], 0.08)
        hero_border = with_alpha(t["focus"], 0.44)
        hero_edge_top = with_alpha("#ffffff", 0.22)
        hero_edge_bottom = with_alpha("#8befff", 0.16)

        card_top = with_alpha("#edf6ff", 0.10)
        card_bottom = with_alpha("#9bb0c6", 0.05)
        muted_top = with_alpha("#eaf4fc", 0.10)
        muted_bottom = with_alpha("#9ab0c7", 0.06)
        footer_top = with_alpha("#f0f7ff", 0.10)
        footer_bottom = with_alpha("#9caec5", 0.06)
        card_border = with_alpha("#eaf6ff", 0.22)
        muted_border = with_alpha("#eaf6ff", 0.18)
        line = with_alpha("#d8e7f6", 0.12)
        card_edge_top = with_alpha("#ffffff", 0.20)
        card_edge_bottom = with_alpha("#7de4f8", 0.12)
        muted_edge_top = with_alpha("#ffffff", 0.16)
        muted_edge_bottom = with_alpha("#7bd8eb", 0.10)
        footer_edge_top = with_alpha("#ffffff", 0.16)
        footer_edge_bottom = with_alpha("#7ddff4", 0.10)
        ...
        chrome_title = "#dce8f3"
        chrome_icon = "#86e9ff"
        chrome_bg_top = with_alpha("#edf6ff", 0.08)
        chrome_bg_bottom = with_alpha("#95aac3", 0.05)
        chrome_border = with_alpha("#eef8ff", 0.14)
        chrome_edge_top = with_alpha("#ffffff", 0.18)
        chrome_edge_bottom = with_alpha("#8fe9ff", 0.10)
        ...
        input_bg = with_alpha("#0a1320", 0.34)
        input_fg = "#eff8ff"
        input_border = with_alpha("#e7f5ff", 0.12)
        input_hover = with_alpha(t["focus"], 0.46)
        input_focus = with_alpha(t["focus"], 0.82)
        input_focus_bg = with_alpha("#0e1824", 0.42)
        input_focus_glow = with_alpha(t["focus"], 0.24)
        input_disabled_fg = with_alpha("#9db0c4", 0.74)
        input_disabled_bg = with_alpha("#0b121c", 0.18)
        input_disabled_border = with_alpha("#e7f5ff", 0.06)
        dropdown_bg = with_alpha("#0f1825", 0.34)
        dropdown_panel_top = with_alpha("#173043", 0.48)
        dropdown_panel_bottom = with_alpha("#09131e", 0.28)
        dropdown_border = with_alpha("#8befff", 0.18)
        selection_bg = t["focus"]
        selection_fg = "#08111a"
        dropdown_hover = with_alpha("#8befff", 0.10)
        dropdown_hover_border = with_alpha("#8befff", 0.18)
        dropdown_selection = with_alpha("#8befff", 0.14)
        dropdown_selection_top = with_alpha("#8befff", 0.24)
        dropdown_selection_bottom = with_alpha("#d7f7ff", 0.10)
        dropdown_selection_border = with_alpha("#8befff", 0.26)
        ...
        scale_selector_bg = with_alpha("#0d1723", 0.34)
        scale_selector_border = with_alpha("#e8f6ff", 0.12)
        scale_pill_bg = with_alpha("#0b141f", 0.34)
    ...
    QListView[glass_popup="true"] {{
        background: {_qss_vertical_gradient(dropdown_panel_top, dropdown_panel_bottom)};
        color: {value};
        border: 1px solid {dropdown_border};
        border-radius: {s(14, 10)}px;
    ...
    QListView[glass_popup="true"]::viewport {{
        background: transparent;
        border-radius: {s(14, 10)}px;
    }}

    QListView[glass_popup="true"]::item {{
        background: transparent;
        border: 1px solid transparent;
        border-radius: {s(11, 8)}px;
        margin: {s(3, 2)}px {s(5, 4)}px;
        padding: {s(9, 7)}px {s(12, 9)}px;
        min-height: {s(30, 24)}px;
    }}

    QListView[glass_popup="true"]::item:hover {{
        background: {dropdown_hover};
        border: 1px solid {dropdown_hover_border};
    }}

    QListView[glass_popup="true"]::item:selected {{
        background: {_qss_vertical_gradient(dropdown_selection_top, dropdown_selection_bottom)};
        border: 1px solid {dropdown_selection_border};
        color: {selection_fg};
    }}
    ...
    QLabel[role="perf_ghost"] {{
        background: transparent;
        border: none;
        color: {with_alpha(mono, 0.52 if dark else 0.42)};
    ...
    QLabel[role="perf_ghost"][state="warn"] {{
        color: {perf_warn};
    }}

    QLabel[role="perf_ghost"][state="error"] {{
        color: {perf_error};
    }}

    QLabel[role="perf_ghost"][has_issue="true"] {{
        font-weight: 620;
    }}
