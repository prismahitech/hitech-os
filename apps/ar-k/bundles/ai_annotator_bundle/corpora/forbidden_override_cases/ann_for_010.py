...
PATH_EXAMPLES = ['reports/annotations/ann_for_010_annotations.json', 'reports_real/annotations/ann_for_010_do_not_write.json', 'reports/registries/ann_for_010_must_not_exist.json']
EVIDENCE_LINES = ['Flags evidence from checkout toggles with single_source trace shape.', 'Current upstream validation state is clean and ambiguity focus is naming collision.', 'Primary risk under review is false certainty; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for ops.client_010.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: switch traces may be cited but never rewritten by annotations.', 'Note 2: advisory prose must remain safely ignorable by canonical writers.', 'Note 3: registry_index is the portable name even when legacy query_index shows up in older notes.', 'Note 4: reports_real is treated as safe-ignore runtime exhaust, not evidence input.', 'Note 5: annotation_index is lookup sugar for advice, not a second registry throne.']
REVIEW_NOTES = ['switch traces may be cited but never rewritten by annotations', 'advisory prose must remain safely ignorable by canonical writers', 'registry_index is the portable name even when legacy query_index shows up in older notes', 'reports_real is treated as safe-ignore runtime exhaust, not evidence input', 'annotation_index is lookup sugar for advice, not a second registry throne']
