...
PATH_EXAMPLES = ['reports/annotations/ann_for_013_annotations.json', 'reports_real/annotations/ann_for_013_do_not_write.json', 'reports/registries/ann_for_013_must_not_exist.json']
EVIDENCE_LINES = ['Records evidence from identity linking with single_source trace shape.', 'Current upstream validation state is gate_hold and ambiguity focus is late validation note.', 'Primary risk under review is legacy alias confusion; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for identity.client_013.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: bundle verify may emit sample JSON, while shipped corpus stays Python-first.', 'Note 2: advisory prose must remain safely ignorable by canonical writers.', 'Note 3: installer state is isolated beneath .ark_install/ai_annotator_bundle to avoid cross-tool spaghetti.', 'Note 4: switch traces may be cited but never rewritten by annotations.', 'Note 5: upstream gate outcome is evidence, never a thing to overwrite.']
