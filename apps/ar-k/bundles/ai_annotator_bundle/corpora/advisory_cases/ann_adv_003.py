...
PATH_EXAMPLES = ['reports/annotations/ann_adv_003_annotations.json', 'reports_real/annotations/ann_adv_003_do_not_write.json', 'reports/registries/ann_adv_003_must_not_exist.json']
EVIDENCE_LINES = ['Captures evidence from identity linking with single_source trace shape.', 'Current upstream validation state is gate_hold and ambiguity focus is late validation note.', 'Primary risk under review is legacy alias confusion; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for search.service_003.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: advisory prose must remain safely ignorable by canonical writers.', 'Note 2: annotation_index is lookup sugar for advice, not a second registry throne.', 'Note 3: upstream gate outcome is evidence, never a thing to overwrite.', 'Note 4: bundle verify may emit sample JSON, while shipped corpus stays Python-first.', 'Note 5: AI Annotator may summarize conflicts, but cannot resolve ownership boundaries.']
REVIEW_NOTES = ['advisory prose must remain safely ignorable by canonical writers', 'annotation_index is lookup sugar for advice, not a second registry throne', 'upstream gate outcome is evidence, never a thing to overwrite', 'bundle verify may emit sample JSON, while shipped corpus stays Python-first', 'AI Annotator may summarize conflicts, but cannot resolve ownership boundaries']
