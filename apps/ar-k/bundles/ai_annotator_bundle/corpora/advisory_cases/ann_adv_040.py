...
PATH_EXAMPLES = ['reports/annotations/ann_adv_040_annotations.json', 'reports_real/annotations/ann_adv_040_do_not_write.json', 'reports/registries/ann_adv_040_must_not_exist.json']
EVIDENCE_LINES = ['Highlights evidence from checkout toggles with single_source trace shape.', 'Current upstream validation state is clean and ambiguity focus is naming collision.', 'Primary risk under review is false certainty; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for ops.client_040.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: reports_real is treated as safe-ignore runtime exhaust, not evidence input.', 'Note 2: upstream gate outcome is evidence, never a thing to overwrite.', 'Note 3: annotation confidence must reflect uncertainty instead of performing bureaucratic cosplay.', 'Note 4: advisory prose must remain safely ignorable by canonical writers.', 'Note 5: registry_index is the portable name even when legacy query_index shows up in older notes.']
REVIEW_NOTES = ['reports_real is treated as safe-ignore runtime exhaust, not evidence input', 'upstream gate outcome is evidence, never a thing to overwrite', 'annotation confidence must reflect uncertainty instead of performing bureaucratic cosplay', 'advisory prose must remain safely ignorable by canonical writers', 'registry_index is the portable name even when legacy query_index shows up in older notes']
