...
PATH_EXAMPLES = ['reports/annotations/ann_adv_044_annotations.json', 'reports_real/annotations/ann_adv_044_do_not_write.json', 'reports/registries/ann_adv_044_must_not_exist.json']
EVIDENCE_LINES = ['Highlights evidence from search ranking with single_source trace shape.', 'Current upstream validation state is ambiguous and ambiguity focus is naming collision.', 'Primary risk under review is false certainty; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for catalog.repository_044.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: bundle verify may emit sample JSON, while shipped corpus stays Python-first.', 'Note 2: reports_real is treated as safe-ignore runtime exhaust, not evidence input.', 'Note 3: switch traces may be cited but never rewritten by annotations.', 'Note 4: annotation confidence must reflect uncertainty instead of performing bureaucratic cosplay.', 'Note 5: registry_index is the portable name even when legacy query_index shows up in older notes.']
REVIEW_NOTES = ['bundle verify may emit sample JSON, while shipped corpus stays Python-first', 'reports_real is treated as safe-ignore runtime exhaust, not evidence input', 'switch traces may be cited but never rewritten by annotations', 'annotation confidence must reflect uncertainty instead of performing bureaucratic cosplay', 'registry_index is the portable name even when legacy query_index shows up in older notes']
