...
PATH_EXAMPLES = ['reports/annotations/ann_adv_006_annotations.json', 'reports_real/annotations/ann_adv_006_do_not_write.json', 'reports/registries/ann_adv_006_must_not_exist.json']
EVIDENCE_LINES = ['Flags evidence from catalog caching with single_source trace shape.', 'Current upstream validation state is warning and ambiguity focus is naming collision.', 'Primary risk under review is false certainty; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for checkout.service_006.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: registry_index is the portable name even when legacy query_index shows up in older notes.', 'Note 2: advisory prose must remain safely ignorable by canonical writers.', 'Note 3: switch traces may be cited but never rewritten by annotations.', 'Note 4: annotation confidence must reflect uncertainty instead of performing bureaucratic cosplay.', 'Note 5: reports_real is treated as safe-ignore runtime exhaust, not evidence input.']
REVIEW_NOTES = ['registry_index is the portable name even when legacy query_index shows up in older notes', 'advisory prose must remain safely ignorable by canonical writers', 'switch traces may be cited but never rewritten by annotations', 'annotation confidence must reflect uncertainty instead of performing bureaucratic cosplay', 'reports_real is treated as safe-ignore runtime exhaust, not evidence input']
