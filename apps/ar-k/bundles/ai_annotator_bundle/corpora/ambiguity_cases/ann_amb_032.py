...
PATH_EXAMPLES = ['reports/annotations/ann_amb_032_annotations.json', 'reports_real/annotations/ann_amb_032_do_not_write.json', 'reports/registries/ann_amb_032_must_not_exist.json']
EVIDENCE_LINES = ['Flags evidence from ops runbooks with legacy_alias trace shape.', 'Current upstream validation state is error_shadow and ambiguity focus is late validation note.', 'Primary risk under review is legacy alias confusion; annotator remains non-authoritative.', 'Legacy index alias handling maps query_index.json to registry_index.json for catalog.repository_032.', 'Observed paths include reports_real quarantine examples that must be ignored by policy.', 'Note 1: annotation_index is lookup sugar for advice, not a second registry throne.', 'Note 2: registry_index is the portable name even when legacy query_index shows up in older notes.', 'Note 3: advisory prose must remain safely ignorable by canonical writers.', 'Note 4: bundle verify may emit sample JSON, while shipped corpus stays Python-first.', 'Note 5: installer state is isolated beneath .ark_install/ai_annotator_bundle to avoid cross-tool spaghetti.']
REVIEW_NOTES = ['annotation_index is lookup sugar for advice, not a second registry throne', 'registry_index is the portable name even when legacy query_index shows up in older notes', 'advisory prose must remain safely ignorable by canonical writers', 'bundle verify may emit sample JSON, while shipped corpus stays Python-first', 'installer state is isolated beneath .ark_install/ai_annotator_bundle to avoid cross-tool spaghetti']
