...
class TemplateConsoleWindow(QDialog):
    ...
    def _init_performance_visualizer(self) -> None:
        ...
        if self._perf_timer is None:
            ...
            self._perf_timer.setInterval(900)
    ...
    def _refresh_performance_visualizer(self) -> None:
        ...
        fps = int(round(float(snapshot.get("fps", 0.0) or 0.0)))
        paint_ms = float(snapshot.get("paint_ms", 0.0) or 0.0)
        motion_interval_ms = int(round(float(snapshot.get("motion_interval_ms", 0.0) or 0.0)))
        is_active = bool(snapshot.get("active", True))

        if fps > 0 and paint_ms > 0.0:
            text = f"{fps:02d} fps · {paint_ms:.1f} ms"
        elif motion_interval_ms > 0:
            estimated_fps = max(1, int(round(1000.0 / max(1, motion_interval_ms))))
            text = f"{estimated_fps:02d} fps · live"
        else:
            text = "perf · idle"

        if not is_active and "idle" not in text:
            text += " · idle"
