...
def build_stylesheet(theme_id: str, scale_id: str = "100") -> str:
    ...
    QMessageBox QPushButton[variant="primary"]:checked {{
        color: {selection_fg};
        background: {_qss_vertical_gradient(primary_hover_top, primary_hover_bottom)};
        border: 1px solid {input_focus};
    }}
    ...
    QMessageBox QPushButton[variant="secondary"]:checked {{
        color: {selection_fg};
        background: {_qss_vertical_gradient(primary_top, primary_bottom)};
        border: 1px solid {primary_border};
    }}
    ...
    QMessageBox QPushButton[variant="secondary"]:checked:hover {{
        background: {_qss_vertical_gradient(primary_hover_top, primary_hover_bottom)};
        border: 1px solid {input_hover};
    }}
    ...
    QScrollBar::sub-page {{
        ...
        QFrame[chart_slot="root"] {{
            background: transparent;
            border: none;
        }}
    
        QFrame[chart_slot="state_card"],
        QFrame[chart_slot="content_surface"] {{
            border-radius: 18px;
            border: 1px solid {card_border};
            background: {_qss_vertical_gradient(card_top, card_bottom)};
        }}
    
        QLabel[chart_slot_role="eyebrow"] {{
            color: {eyebrow};
            font-size: 10px;
            font-weight: 760;
        ...
        QLabel[chart_slot_role="glyph"] {{
            color: {field};
            font-size: 20px;
            font-weight: 780;
            border-radius: 12px;
            border: 1px solid {with_alpha(t["focus"], 0.24 if dark else 0.28)};
            background: {with_alpha(t["focus"], 0.10 if dark else 0.12)};
            padding: 0px;
        }}
    
        QLabel[chart_slot_role="title"] {{
            color: {title};
            font-size: 18px;
            font-weight: 820;
            line-height: 1.2em;
        ...
        QLabel[chart_slot_role="subtitle"] {{
            color: {hint};
            font-size: 11px;
            line-height: 1.3em;
        }}
    
        QLabel[chart_slot_role="bullet"] {{
            color: {value};
            font-size: 11px;
            line-height: 1.35em;
            padding-left: 4px;
