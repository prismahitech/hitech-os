...
from PySide6.QtWidgets import QFrame, QHBoxLayout, QLabel, QPlainTextEdit, QSpinBox, QVBoxLayout, QWidget
...
class ControlsPanel(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        action_row_two.addWidget(create_button("Ingest", "secondary", self.ingestRequested.emit, icon="spark", parent=self), 1)
        action_row_two.addWidget(create_button("Repair", "secondary", self.repairRequested.emit, icon="settings", parent=self), 1)

        action_row_three = QHBoxLayout()
        action_row_three.setSpacing(8)
        layout.addLayout(action_row_three)
        action_row_three.addWidget(create_button("Demo", "secondary", self.loadDemoRequested.emit, icon="spark", parent=self), 1)
        action_row_three.addWidget(create_button("Init DB", "secondary", self.initDbRequested.emit, icon="settings", parent=self), 1)
        ...
        self.source_label = QLabel("Sources: not resolved", self)
        self.source_label.setProperty("role", "hint")
        self.source_label.setWordWrap(True)
        layout.addWidget(self.source_label)

        self.help_label = QLabel(
            "Ctrl+R refreshes metrics, Ctrl+L focuses search, Ctrl+Shift+C toggles charts, Ctrl+E repairs storage, Ctrl+D loads demo data, and Ctrl+I runs ingest.",
            self,
        )
        ...
        self.diagnostics.setMinimumHeight(120)
        self.diagnostics.setPlaceholderText("Runtime diagnostics and controller events land here.")
    ...
    def set_query_text(self, text: str) -> None:
        self.query_entry.setText(str(text or ""))
    ...
    def set_runtime_info(self, *, root_path: str, db_path: str, source_paths: Iterable[str] = ()) -> None:
        self.root_label.setText(f"Root: {root_path}")
        self.db_label.setText(f"DB: {db_path}")
        joined = ", ".join(str(item) for item in source_paths if str(item).strip()) or "none"
        self.source_label.setText(f"Sources: {joined}")

    def set_scope_text(self, text: str, *, tone: str = "neutral") -> None:
        self.scope_chip.setText(text)
        self.scope_chip.setProperty("tone", tone)
        repolish(self.scope_chip)
    ...
    def push_diagnostic(self, line: str) -> None:
        chunk = str(line or "").strip()
        if not chunk:
            return
        current = self.diagnostics.toPlainText().strip()
        if current:
            self.diagnostics.setPlainText(current + "\n" + chunk)
        else:
            self.diagnostics.setPlainText(chunk)
        self.diagnostics.verticalScrollBar().setValue(self.diagnostics.verticalScrollBar().maximum())
