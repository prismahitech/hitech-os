...
import { buildEventsExport, csvResponse } from "@/server/pos-export";
...
try {
    ...
    const result = await buildEventsExport(input);
    ...
    return ok(result.data, undefined, {
      endpoint: "GET /api/pos/export/events",
      format: input.format,
      businessId: input.businessId,
      audit
    });
