...
import { buildSalesTodayExport, csvResponse } from "@/server/pos-export";
...
try {
    ...
    const result = await buildSalesTodayExport(input);
    ...
    const audit = tabletAuditMeta("export.local.create", {
      ...
      entityType: "SalesTodayExport",
      entityId: result.filename,
      after: { format: input.format, rowCount: result.data.rows.length, date: result.data.date }
    });
    ...
    return ok(result.data, undefined, {
      endpoint: "GET /api/pos/export/sales-today",
      format: input.format,
      businessId: input.businessId,
      audit
    });
